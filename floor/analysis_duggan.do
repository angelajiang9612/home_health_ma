...
foreach var in `outcomes' {
	ivreghdfe `var' pop2007_metro pop2007_metro_sq pop2007_cty pop2007_cty_sq ffs_average `controls' (penetration=urban) [aweight= pop_hth] if group==1 & CON==0, absorb(year) cluster(cbsa) 
}

foreach var in `outcomes' {
	ivreghdfe `var' pop2007_metro pop2007_metro_sq pop2007_cty pop2007_cty_sq ffs_average `controls' (penetration=urban) if inlist(group,1,2) & CON==0 [aweight= pop_hth] , absorb(year) cluster(cbsa) 
}


foreach var in `outcomes' {
	ivreghdfe `var' pop2007_metro pop2007_metro_sq pop2007_cty pop2007_cty_sq ffs_average `controls' (penetration=urban) [aweight= pop_hth] if group==1 & CON==1, absorb(year) cluster(cbsa) 
}

foreach var in `outcomes' {
	ivreghdfe `var' pop2007_metro pop2007_metro_sq pop2007_cty pop2007_cty_sq ffs_average `controls' (penetration=urban) if inlist(group,1,2) & CON==1 [aweight= pop_hth] , absorb(year) cluster(cbsa) 
}
