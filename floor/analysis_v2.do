...
//drop if penetration<=5 //No MA presence at all or less than 5 percent -doing this probably does similar stuff to sample selection as the original paper. Basically start with those with MA_presence. (can also frame as heterogeneity) -not doing this seem to make estimates look larger
...
xtile ma_quart = penetration, nq(4)
...
//all of these seem smaller than combining the entire sample//
reghdfe penetration b2000.year##c.distance50 if ma_quart==1 [pweight= eligibles_2000], absorb(countySSA) cluster(countySSA) 

reghdfe penetration b2000.year##c.distance50 if ma_quart==2 [pweight= eligibles_2000], absorb(countySSA) cluster(countySSA) 

reghdfe penetration b2000.year##c.distance50 if ma_quart==3 [pweight= eligibles_2000], absorb(countySSA) cluster(countySSA) 

reghdfe penetration b2000.year##c.distance50 if ma_quart==4 [pweight= eligibles_2000], absorb(countySSA) cluster(countySSA) 

///First stage for simple MA penetration on D with some controls (not the ones in literature yet, see if can add them) -try years 2006-2019 only 
...
//after including the years after 2004 (inc) it affected estimate for the years before..? pre-trend of decreasing..?  //estimates become smaller 



//looks a bit weird but seems to indicate okay first stage. But pre-trend seems to worsen as I included more years into the analysis.  
